...
# plt.subplot(2,3,2)
# plt.imshow(RedMask, cmap = 'Reds')
# plt.title("Red Map")

# plt.subplot(2,3,3)
# plt.imshow(GreenMask, cmap = 'Greens')
# plt.title("Green Map")

# plt.subplot(2,3,4)
# plt.imshow(BlueMask, cmap = 'Blues')
# plt.title("Blue Map")
...
# green_minus_grey = GreenMask.astype(np.int16) - BlueMask.astype(np.int16) 



# for i in range(len(GreenMask[:,0])):
#     for j in range(len(GreenMask[0,:])):
#         if green_minus_grey[i,j] < 0 :
#             green_minus_grey[i,j] = 0
            

# plt.figure(2)
# plt.imshow(green_minus_grey, cmap = 'grey')
# plt.title("Green mask - Grey Mask")
# plt.show()
...
roads = green_minus_morph[20:685,73:815]
...
for i in range(len(roads[:,0])):
    for j in range(len(roads[0,:])):
        if roads[i,j] ==1:
            arr[int(i//35), int(j//30.916666)] = 1
